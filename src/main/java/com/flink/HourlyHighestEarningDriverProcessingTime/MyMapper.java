...
public class MyMapper implements MapFunction<String, Tuple2<TaxiPojo, Integer>>{
	...
	public Tuple2 map(String value) throws Exception {
		...
		return new Tuple2<TaxiPojo, Integer>(new TaxiPojo(data[0],data[1] , Long.parseLong(data[2]), data[3], Long.parseLong(data[4]),Long.parseLong( data[5]),Integer.parseInt(data[6]) ),1);
	}
}
